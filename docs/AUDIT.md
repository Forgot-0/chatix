...
>
> **Обновление:** бэклог закрыт целиком — все четыре яруса (T-1…T-29). Состояние после них —
> `flutter analyze` 0 issues, `flutter test` **437 passed**. Таблицы разделов 1–4
...
### Ярус 3 — дорого, но обязательно ✅ СДЕЛАНО

| # | Задача | Статус | Что получилось |
|---|---|---|---|
| T-15 | Миграция строк в ARB | ✅ | Было 148 захардкоженных строк в виджетах — стало **0**. 236 ключей во всех 6 локалях; переведены и легаси-ключи, которые реально рендерятся (`theme`, `change_language`, `notifications`), и целиком заполнен `intl_fr.arb`. Непереведёнными остаются 22 ключа на локаль — все они принадлежат демо-экранам `lib/examples/**` и в продукте не встречаются |
| T-16 | Айдентика ChatiX | ✅ | [chatix_palette.dart](lib/core/theme/chatix_palette.dart) — violet #6E56F8 / mint #22C7A9 / coral #F2624F / amber #F5A524 и тёплые графитовые нейтрали вместо сине-серых. [app_theme_extension.dart](lib/core/theme/app_theme_extension.dart) — ThemeExtension с градиентом исходящего пузыря, асимметричными радиусами 20/20/20/6, тремя режимами плотности и единой пружиной (`easeOutCubic`, 220 мс). Manrope в `assets/fonts` + табличные цифры на всех числовых стилях. Цвет автора — детерминированно из `user_id`, 8 палитр с отдельными значениями для светлой и тёмной. Обои — процедурные mesh-градиенты ([chat_wallpaper.dart](lib/features/chat/presentation/widgets/chat_wallpaper.dart)), ноль байт ассетов, seed от `chat_id`. Переходы экранов — shared-axis |
| T-17 | Паттерны мессенджера | ✅ | Группировка подряд идущих сообщений (якорный угол только у последнего в блоке), липкий разделитель дат с «Сегодня/Вчера», разделитель «непрочитанные» по `last_read_message_seq`, плавающая кнопка «вниз» со счётчиком, свайп-по-сообщению → ответ (с порогом, хаптиком и пружиной), правка в поле ввода с баннером вместо `AlertDialog`, «расцветание» чипсов реакций. Логика группировки вынесена в [message_grouping.dart](lib/features/chat/presentation/utils/message_grouping.dart) и покрыта тестами |
| T-18 | Настоящий FCM/APNs | ⚠️ код готов, нужен конфиг | [firebase_notification_service.dart](lib/core/notifications/firebase_notification_service.dart): реальные `firebase_messaging` + каналы через `flutter_local_notifications`, foreground/background/cold-start, разрешения, топики. **Главное исправлено безусловно:** фейковый `debug-token-<ts>` больше не уходит в `POST /devices/` — при отсутствии конфига сервис отдаёт `null`, а `RegisterDeviceUseCase` пустой токен не регистрирует. Стенд-ин включается только флагом `--dart-define=CHATIX_DEBUG_NOTIFICATIONS=true`. Что нужно от владельца проекта — ниже, в разделе 6 |
| T-19 | Голосовые сообщения | ✅ | Пакет `record`, AAC/m4a (`audio/mp4` есть в белом списке §5.5). Удержание микрофона, свайп-влево — отмена, свайп-вверх — фиксация, живой уровень входа, обратный отсчёт последних 30 секунд и жёсткий стоп на 600 с. Отправка через `attachment_type: "voice"` + `message_type: "voice"`. Воспроизведение — [voice_player.dart](lib/features/chat/presentation/widgets/voice_player.dart) с перематываемой waveform (форма детерминирована по id вложения — бэкенд waveform не отдаёт). Разрешения прописаны в манифестах обеих платформ |
| T-20 | Видео и кружки | ✅ | Пакет `video_player`. [video_preview.dart](lib/features/chat/presentation/widgets/video_preview.dart): постер-кадр с бейджем play и длительностью, полноэкранный просмотр, `video_note` рендерится кругом с зацикливанием. Ссылка перевыпускается перед инициализацией — инлайновая живёт 300 с (§5.5). **Запись** кружков не делалась: аудит определял T-20 как «превью и воспроизведение», для съёмки нужен `camera` и отдельный UI |

**Тесты, добавленные ярусом:** +43 (369 → 412). Три поведенческие правки проверены мутацией
(T-16 цвет автора, T-17 группировка, T-19 отсечение промаха).

- [chatix_theme_test.dart](test/core/theme/chatix_theme_test.dart) — палитра, геометрия пузыря, плотность, табличные цифры
- [message_grouping_test.dart](test/features/chat/presentation/utils/message_grouping_test.dart) — окно группировки, полночь, системные сообщения, граница непрочитанного
- [voice_recorder_test.dart](test/features/chat/presentation/providers/voice_recorder_test.dart) — разрешения, фиксация, отмена, лимит 600 с

**Найдено и починено по дороге:** `_SwipeToReply` держал `late final AnimationController`,
который не использовался, но **создавался внутри `dispose()`**, если по сообщению ни разу не
свайпнули — `createTicker` при этом лезет к предку деактивированного элемента и роняет assertion.
Нашлось при рендере экрана в обеих темах.

### Ярус 4 — мелочи и гигиена ✅ СДЕЛАНО

| # | Задача | Статус | Что получилось |
|---|---|---|---|
| T-21 | Дедуп direct-чатов | ✅ | [direct_chat_lookup.dart](lib/features/chat/presentation/utils/direct_chat_lookup.dart) ищет существующий 1:1 до `POST /chats/` — во всех трёх точках создания (профиль, поиск, экран создания). Ростер авторитетнее `last_message`: при расхождении промах, иначе чужой чат молча переиспользовался бы как переписка с этим человеком. Чтение `DIRECT_CHAT_EXISTS` оставлено ремнём безопасности |
| T-22 | `ws.error NOT_CHAT_MEMBER` виден пользователю | ✅ | У `ws.error` нет `channel` (§6.4), поэтому событие некому приписать — сервис проставляет чат из последнего отправленного `subscribe`, то есть из запроса, на который пришёл отказ. Экран показывает баннер «живые обновления выключены», а чат снимается с отслеживания: переподписываться на него при каждом реконнекте значило бы вечно получать тот же отказ |
| T-23 | `initial_chat_id`/`initial_last_seq` | ✅ | Реконнект просит гейтвей подписать нас прямо из URL — минус round-trip. Срабатывает только когда чат ровно один и курсор известен (оба параметра идут вместе или никак, §6.1), и `_resumeSubscriptions` этот чат пропускает, иначе `ws.history` пришла бы дважды |
| T-24 | Комментарий при форварде | ✅ | Диалог стал выбором цели + поле комментария, возвращает `ForwardTarget`. В bulk-форварде комментарий уходит только с первым сообщением — повторять его на каждом было бы спамом в чате-получателе |
| T-25 | «Мои устройства» | ✅ | `GET /users/sessions/` (голый массив, §0.16) → модель → репозиторий → usecase с сортировкой (активные выше, свежие раньше) → экран + строка в настройках. **Read-only осознанно:** завершение сессии — это `DELETE /sessions/{id}/` с админским правом `user:update` (§3.15), кнопка «выйти на этом устройстве» у обычного пользователя давала бы только 403 |
| T-26 | Семантика `banned_to` | ✅ | Вместо одного date-picker'а — три явных варианта: «навсегда» (null), «до даты» (будущее), «снять бан» (прошлое). Три состояния поля из §5.3 больше не надо угадывать |
| T-27 | Аватар чата | ⛔ невозможно клиентом | `ChatDTO.avatar_s3_key` приходит, но в §5 нет ни эндпоинта загрузки аватара чата, ни presigned-URL для этого ключа — превратить ключ в картинку нечем. Перепроверено, остаётся требованием к бэкенду |
| T-28 | Демо вне прод-сборки | ✅ | Маршрут `LocalizationAssetsDemo` завёрнут в `if (kDebugMode)` — это compile-time константа, поэтому в release он и всё, что он тянет из `lib/examples`, вырезается tree-shaking'ом |
| T-29 | **V-15**: зависание на `provider.future` | ✅ | Опрос аватара больше не ждёт `.future` (который для брошенного `Failure` не завершается никогда), а читает `AsyncValue` с ограниченным ожиданием. Цикл ретраев снова работает как задуман — а он и написан в расчёте на неудачу, штатный 404 сразу после регистрации (§4.1) |

**Тесты, добавленные ярусом:** +25 (412 → 437). Четыре правки проверены мутацией
(T-21, T-22, T-23, T-25).

- [direct_chat_lookup_test.dart](test/features/chat/presentation/utils/direct_chat_lookup_test.dart) — оба источника, приоритет ростера, отказ гадать по своему же сообщению
- [chat_socket_handshake_test.dart](test/core/websocket/chat_socket_handshake_test.dart) — сокращение handshake и его отсутствие при нескольких чатах, атрибуция `ws.error`
- [get_my_sessions_use_case_test.dart](test/features/auth/domain/usecases/get_my_sessions_use_case_test.dart) — порядок сессий, отсутствующие метки времени, фолбэки названия

> Первая версия теста T-21 мутацию пережила: `peerProfile` и сам предпочитает ростер,
> так что удаление `continue` ничего не меняло на том наборе данных. Добавлен случай, где
> источники действительно расходятся — частичный ростер без собеседника при его же
> `last_message`; теперь охранник нагружен и проверен.

---

## 6. Что требует бэкенда

Ничего из перечисленного нельзя закрыть на клиенте — это ограничения API.

| Потребность | Состояние |
|---|---|
| **Конфиг Firebase** | Код push готов и включён по умолчанию, но `Firebase.initializeApp()` без `android/app/google-services.json`, `ios/Runner/GoogleService-Info.plist` и плагина `com.google.gms.google-services` в Gradle падает — сервис это ловит и молча отключает push, ничего не регистрируя. Файлы выпускает владелец Firebase-проекта; после их добавления T-18 заработает без единой правки в Dart |
| Поиск по сообщениям | Эндпоинта нет. `chat_search_screen` ищет людей через `GET /profiles/` и фильтрует уже загруженные чаты локально |
| Обнаружение публичных чатов | `POST /chats/{id}/join/` есть, списка публичных чатов нет — присоединиться можно только зная `chat_id`. Из-за этого `joinChatUseCase` так и остаётся без UI |
| Аватар чата | `ChatDTO.avatar_s3_key` отдаётся, но нет ни эндпоинта загрузки, ни способа получить по ключу URL (T-27) |
| Индикатор набора текста | `typing_start`/`typing_stop` объявлены в `WSEventType`, бэкендом не публикуются (§6.4) |
| События звонка | `call_started`/`call_ended`/`call_joined`/`call_left` объявлены, не публикуются. REST «завершить звонок» тоже нет — выход только через disconnect в LiveKit SDK (§5.6) |
| Причина ошибки вложения | Только `attachment_status: "error"`; конкретная `AttachmentRejectionReason` клиенту не передаётся (§5.5) — объяснить пользователю, что именно не так с файлом, нечем |
| Результат обработки аватара | Обратной связи нет, `upload_complete` всегда `200` (§0.10) — остаётся поллинг `GET /profiles/{id}/` |
| OAuth `redirect_uri` / deep link | §3.8 сам просит уточнить схему у бэкенд-команды; без неё обмен `code`+`state` не реализовать, поэтому кнопки скрыты за `oauthSignInEnabled` |
| Дубли direct-чатов | `409 DIRECT_CHAT_EXISTS` не срабатывает (§5.2). Клиент дедупит сам (T-21), но видит только загруженные страницы — гонка двух устройств всё ещё создаст дубль |
| Создатель заперт в чате | `Chat.leave()` сверяется с `created_by`, передачи владения нет (§5.2). UI это объясняет, но снять ограничение может только бэкенд |
| Завершение своей сессии | `DELETE /sessions/{id}/` требует админского `user:update` (§3.15), поэтому экран «Мои устройства» только читает |
| Свои роли и права | Self-эндпоинта нет, только админский `GET /users/` (§3.9) — RBAC-подсказки в UI строить не на чем |
| Забаненные участники | `GET /members/` их скрывает, `GET /chats/{id}/`.members — показывает (§5.3). Несогласованность бэкенда; клиент считает источником истины второе |
| Запись видео-кружков | Не блокер бэкенда, а незакрытая клиентская работа: съёмка `video_note` требует пакета `camera` и отдельного UI. Воспроизведение и превью уже есть (T-20) |
